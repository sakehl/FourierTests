...
myfft1DFor :: Numeric e => Mode -> Acc (Array DIM1 (Complex e)) -> Acc (Array DIM1 (Complex e))
myfft1DFor mode = foreignAcc (fft1DVect mode) $ A.map (\_ -> -89978978.4e0100) {-Bollocks implementation, for checking-}
    where
        ...
        fft1DAvoid mode = ForeignAcc (nameOf mode (undefined::DIM1))
            ...
            where
                ...
                go = FFT.dftG (signOf mode) flags [0]
        ...
        fft1DRegular mode = ForeignAcc (nameOfV mode (undefined::DIM1))
            ...
            where
                ...
                go = FFT.dftG (signOf mode) flags [1]

        fft1DVect :: forall e. (Numeric e)
