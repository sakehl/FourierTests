...
import Data.Array.Accelerate                                        as A hiding (fromInteger, fromRational, fromIntegral, shape, min, size)
...
fft2DForGPU mode = foreignAcc (fft2DVect mode) $ A.map (\_ -> -89978978.4e0100) {-Bollocks implementation, for checking-}
    where
        ...
        fft2DVect mode = VectorisedForeign $  f
            where
                ...
                f AvoidedT AvoidedT = GPUFFT.fft2D mode
...
fft1DVec :: IsFloating e
      => Mode
      -> ForeignAcc (Array DIM2 (Complex e) -> (Array DIM2 (Complex e)))
fft1DVec mode = ForeignAcc "fft2D" $ liftAtoC (cuFFT mode)

fft1DVecW :: (Elt e, IsFloating e, A.RealFloat e) => Mode -> Acc (Array DIM2 (Complex e)) -> Acc (Array DIM2 (Complex e))
fft1DVecW mode = foreignAcc (fft1DVec mode) $ A.map (\_ -> -89978978.4e0100) {-Bollocks implementation, for checking-}
...
fft2DW mode = foreignAcc (PTX.fft2D mode) $ A.map (\_ -> 0) {-Bollocks implementation, for checking-}
...
fft3DForGPU :: (Elt e, IsFloating e, A.RealFloat e) => Mode -> Acc (Array DIM3 (Complex e)) -> Acc (Array DIM3 (Complex e))
fft3DForGPU mode = foreignAcc (PTX.fft3D mode) $ A.map (\_ -> -89978978.4e0100)
...
-- fft2DForGPU :: Numeric e => Mode -> Acc (Array DIM2 (Complex e)) -> Acc (Array DIM2 (Complex e))
-- fft2DForGPU mode = foreignAcc (fft2DVect mode) $ A.map (\_ -> -89978978.4e0100) {-Bollocks implementation, for checking-}
--     where
--         fft2DAvoid :: forall e . (Numeric e) 
--                     => Mode 
--                     -> GPU.ForeignAcc (Array DIM2 (Complex e) -> Array DIM2 (Complex e))
--         fft2DAvoid = fft2D

--         fft2DRegular :: forall e . (Numeric e) 
--                     => Mode 
--                     -> GPU.ForeignAcc (Array DIM3 (Complex e) -> Array DIM3 (Complex e))
--         fft2DRegular mode = GPU.ForeignAcc "cuda.fft2dVec" $ fft' fft2DVec_plans mode

--         fft2DVect :: forall e . (Numeric e)
--                 => Mode -> VectorisedForeign (Array DIM2 (Complex e) -> Array DIM2 (Complex e))
--         fft2DVect mode = VectorisedForeign $  f
--             where
--                 f :: Arrays a' => LiftedType (Array DIM2 (Complex e)) a' -> LiftedType (Array DIM2 (Complex e)) b' -> GPU.ForeignAcc (a' -> b')
